...
def validation(model: nn.Module, testloader: torch.utils.data.Dataloader, criterion: Union[Callable, nn.Module]) -> \
Tuple[float, float]:
    accuracy = 0
    test_loss = 0
...
def train(model: nn.Module, trainloader: torch.utils.data.DataLoader, testloader: torch.utils.data.DataLoader,
          criterion: Union[Callable, nn.Module], optimizer: Optional[torch.optim.optimizer] = None, epochs: int = 5,
